...
export default function ResourcesEditPage() {
  ...
  useEffect(() => {
    ...
    apiCall(`/board/archives/${id}`).then((res) => {
      const d = res.data;
      setTitle(d.title || "");
    ...
    }).catch(() => showAlert("자료를 불러오지 못했습니다."));
  }, [id]);
  ...
  const handleDeleteExisting = async (attNo: number, name: string) => {
    ...
    try {
      await apiCall(`/board/archives/attachments/${attNo}`, { method: "DELETE" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/board/archives/${id}`, {
        method: "PUT",
      ...
      router.push(`/board/resources/${id}`);
    } catch (err: any) {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h2><FolderOpen size={22} className={styles.titleIcon} /> 자료 수정</h2>
