...
고속도로 CCTV 영상과 위험 객체 감지 현황을 실시간으로 확인하는 통합 관제 화면입니다.  
오늘 감지 수, 미확인 감지, 활성 CCTV, AI 정확도, 시스템 상태와 최근 위험 알림을 한 화면에서 확인할 수 있습니다.

![실시간 관제 대시보드](docs/images/real-time-monitoring.png)

### 2. AI 모델 테스트

Keras, YOLOv8, YOLOv11 모델의 추론 결과를 비교하고, Soft Voting을 통해 최종 탐지 결과를 산출하는 테스트 화면입니다.  
모델별 탐지 클래스와 신뢰도를 비교하여 AI 모델의 판단 과정을 확인할 수 있도록 구성했습니다.

![AI 모델 테스트](docs/images/ai-model-test.png)

### 3. 위험구간 지도 분석

누적된 객체 감지 데이터를 지역별로 시각화한 분석 화면입니다.  
지역별 감지 건수, 위험도가 높은 CCTV 지점, 주요 위험 구간을 지도와 순위 형태로 확인할 수 있습니다.

![위험구간 지도 분석](docs/images/risk-area-map.png)
